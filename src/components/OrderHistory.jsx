  ...
  return (
    <div>
      ...
      <table className='overflow-x-auto bg-white w-[94%] mx-auto h-auto rounded-[10px] hidden md:table'>
        ...
        <tbody>
          {latestOrders.map(order => (
            ...
            <tr title='click on order ID to approve' key={order.id} className=' flex py-[20px] px-[5px] md:py-[20px]
      ...
      {latestOrders.map(order => (
        ...
        <Link to={`/dashboard/orders/${order.order_id}`}  title='click here to Approve Order' className='w-[80%] flex flex-col gap-2 p-6 mx-auto bg-white mb-10 rounded-[10px] h-auto flex-shrink-0 border-[0.5px] border-[#7B7B7B]
        ' key={order.id}>

           <div  className='flex gap-4 justify-between'>
            <p className='font-lato text-[16px] font-bold tracking-[0.255px] underline text-[#0D1821]' >Customer:</p>
          ...
          <div  className='flex gap-5 justify-between'>
            <p className='font-lato text-[14px] font-bold tracking-[0.255px] underline text-[#0D1821]'>Date of Order:</p>
