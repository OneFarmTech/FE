...
const CartProduct = ({ item, onRemove, onAddMore }) => {
  ...
  return (
    <article className="bg-white rounded-lg shadow-md flex gap-5 items-center lg:gap-8 p-5 pt-10 relative">
      ...
      <figure className="w-2/4 lg:w-2/5 bg-white">
        <img src={item.image} alt="Product" />
      </figure>
  
      <div className="lg:w-3/5 flex flex-col gap-3 xl:gap-4 ">
        <h1 className="font-semibold  text-green-60 text-xl capitalize">{item.name}</h1>
       {/*<p className="text-xl"> 2kg per basket</p>*/}
        <h3 className="font-medium  text-green-50 text-lg naira-sign">{displayCost()}</h3>
        <div className="flex gap-4 items-center">
         
         {/* <h4 className="font-semibold text-xl line-through italic text-black-50">#8,500</h4>*/}
          <h5 className="text-white bg-green-50 rounded p-1">-12%</h5>
          </div>
          <div className="flex gap-1 lg:gap-3 items-center">
          <button onClick={() => handleRemoveFromCart(item.id)} className="py-1 px-1 bg-green-600 rounded-md text-white"><TiMinus /></button>
          <p className="lg:text-xl text-sm font-semibold  text-green-50 items-end">Qty: {item.quantity} </p>
          <button onClick={() => handleAddToCart(item)} className="py-1 px-1 bg-green-600 rounded-md text-white"><TiPlus /></button>
          </div>
