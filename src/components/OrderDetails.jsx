...
const OrderDetails = () => {
  ...
  const [approveButtonVisible, setApproveButtonVisible] = useState(true);
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      ...
      try {
        ...
        const matchedOrders = orders.filter(order => order.order_id === orderId && order.product.user_id === parseInt(userId));
  ...
  const approveOrder = async () => {
    ...
    try {
      ...
      if(response.data.message == 'Order Approved'){
        Swal.fire({
          title: 'GREAT',
          text: `You have successfully approved this order and product will be shipped to the customer immediately`,
    ...
    Swal.fire({
      title: 'Error!',
      text: 'Failed to approve order. Please try again later.',
      icon: 'error',
      confirmButtonText: 'Okay'
    });
    }
  ...
  const isAnyOrderApproved = orderDetails.some(order => order.status === 'Approved');
  return (
    <div  className='w-[90%] mx-auto mt-10 flex flex-col gap-6'>
      ...
      <div  className='flex flex-col gap-4 w-full md:w-[50%]'>
        <h2 className='font-bold md:text-xl text-[14px] text-[#0D1821] md:tracking-wide  tracking-normal font-lato'>Name of Customer </h2>
      ...
      <div  className='flex flex-col gap-4 w-full md:w-[50%]'>
        <h2 className='font-bold md:text-xl text-[14px] text-[#0D1821] md:tracking-wide  tracking-normal font-lato'>Customer&apos;s Phone</h2>
        ...
        <div className='flex flex-col gap-4 w-full md:w-[50%]'>
          <h2 className='font-bold md:text-xl text-[14px] text-[#0D1821] md:tracking-wide  tracking-normal font-lato'>Customer&apos;s Email </h2>
          <div className='text-green-300  flex w-full h-[64px] items-center bg-[#fff] pt-[20px] pr-[45px] pb-[22px] pl-[28px] rounded-[10px] shadow-md'>
             {buyer.email}
             </div>
        </div>
      ...
      <div  className='flex flex-col gap-4 w-full md:w-[50%]'>
        ...
        <div className='flex flex-col gap-4  w-full md:w-[50%]'>
          ...
          {getFormattedDate(orderDate)}
            ...
            <div  className='mb-10'>
          {isAnyOrderApproved || !approveButtonVisible ? (
        <p className= ' text-red-200 font-bold py-2 px text-lg md:text-2xl w-[50%]'>Order Approved</p>
      ) : (
      <button  className={`${approveButtonVisible ? 'block' : 'hidden'} bg-green-500 hover:bg-green-700 text-white font-bold py-4 px-4 rounded-[10px]  w-full`}
  onClick={approveOrder} > Approve Order
      </button>
      )}
      {/*<button  className=' bg-green-500 hover:bg-green-700  items-center text-white font-bold py-2 px-4 rounded  w-[50%]'
   onClick={() => {
